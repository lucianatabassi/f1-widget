...
function InfoSection({
  ...
  return (
    <ThreeColumns className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-zinc-900/80 to-black backdrop-blur-xl border border-white/5">
      ...
      <div className="relative z-10 flex flex-col justify-center gap-8 px-10 py-8 text-white w-full h-full">
          ...
          <div className="flex flex-col flex-1 h-[338px] justify-start gap-4">
            {sesionesProgramadas.map((sesion, index) => (
              <div
                key={index}
                className={`
    flex items-center gap-6 pb-4
    relative

    ${
      index !== sesionesProgramadas.length - 1
        ? // Si NO es el último elemento, agregamos el pseudo-elemento 'after'
          "after:absolute after:bottom-0 after:left-0 after:h-[1px] after:w-full after:bg-gradient-to-r after:from-transparent after:via-white/5 after:to-transparent"
        : // Si ES el último, no agregamos nada
          ""
    }
  `}
              >
                ...
                <div
                  className={`flex flex-col items-center justify-center w-12 ${
                    sesion.isMain
                      ? "border-l-[3px] border-[#ff0000] pl-2"
                      : "pl-3"
                  }`}
                >
                  <span
                    className={`text-xl font-f1 font-medium leading-none ${
                      sesion.isMain
          ...
          <div
            className="relative w-[428px] h-[338px] shrink-0 rounded-2xl bg-white/2  /* Color base muy transparente (5% de blanco) */
    backdrop-blur-lg     /* El desenfoque fuerte de lo que hay detrás */
    border border-white/10 /* Un borde fino y semitransparente para definir los límites */
     overflow-hidden"
          >
